        ...
        <Tabs defaultValue="all" className="gap-4">
          <TabsList className="bg-transparent rounded-none border-b p-0">
            ...
            >
              Pending
              <Badge className="h-5 min-w-5 rounded-full px-1 text-black dark:text-white bg-accent tabular-nums ml-2">
                {candidates.filter((c) => c.status === CandidateStatus.PENDING).length}
              </Badge>
            </TabsTrigger>
            <TabsTrigger
              value="approved"
            ...
            >
              Approved
              <Badge className="h-5 min-w-5 rounded-full px-1 text-black dark:text-white bg-accent tabular-nums ml-2">
                {candidates.filter((c) => c.status === CandidateStatus.APPROVED).length}
              </Badge>
            </TabsTrigger>
            <TabsTrigger
              value="rejected"
          ...
          <TabsContent value="pending">
            <DataTable
              columns={columns}
              data={candidates.filter((c) => c.status === CandidateStatus.PENDING)}
            />
          </TabsContent>
          <TabsContent value="approved">
            <DataTable
              columns={columns}
              data={candidates.filter((c) => c.status === CandidateStatus.APPROVED)}
            />
          </TabsContent>
