...
const LiveResultsPage: React.FC = () => {
  ...
  // Voters who actually participated in this election (cast at least one vote)
  const votersWhoVotedElection = new Set(
    election.votes.map((v: any) => v.voterId)
  );
  ...
  const positionResults = election.positions.map((position) => {
    ...
    // Distinct voters who cast at least one vote for this position
    const uniqueVotersForPosition = new Set(
      position.votes.map((v: any) => v.voterId)
    ).size;

    // Abstain count per position = voters who voted in the election
    // minus voters who cast a vote for this specific position
    const abstainCount = Math.max(
      0,
      votersWhoVotedElection.size - uniqueVotersForPosition
    );

    return { position, candidates: sortedCandidates, totalVotes, abstainCount };
  });
  ...
  return (
    <div>
      ...
      {/* Position Results */}
      {positionResults.map(({ position, candidates, totalVotes, abstainCount }) => {
        return (
          <Card
            key={position.id}
