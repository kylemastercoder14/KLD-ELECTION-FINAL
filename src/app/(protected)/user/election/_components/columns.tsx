...
import { CheckIcon, ChevronsUpDown, CopyIcon, PlusCircle } from "lucide-react";
...
export const columns: ColumnDef<ElectionWithProps>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      return (
        <Link
          href={`/user/election/${row.original.id}`}
          className={`ml-2.5 text-xs px-1 py-0.5 ${buttonVariants({ variant: "default", size: "sm" })}`}
        >
